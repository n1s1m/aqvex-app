        ...
        <Card className="flex flex-col justify-between shadow-none max-w-[347px] w-full gap-0 flex-1 max-sm:max-w-full max-sm:px-4 rounded-none overflow-visible pt-0">
            <CardHeader className="p-0 border-0 border-b-0 bg-transparent gap-0">
                ...
                <CardTitle className="flex items-baseline gap-2 pt-4.5">
                    ...
                    <span className="text-gradient-main font-medium">{product.price} {product.currency}</span>
                    {hasDiscount && <DiscountBadge value={product.discount_percent} />}
                </CardTitle>
                <CardDescription className="pt-8">
                    <span className="text-primary text-lg font-medium">{product.name}</span>
                </CardDescription>
            </CardHeader>
            <CardContent className="p-0 border-0 border-b-0 bg-transparent pt-4">
                ...
                <div className="flex items-center gap-2 flex-row pt-8">
                    <p className="text-primary text-sm flex items-center gap-3.5 flex-row">
                        ...
                        <Image src="/tear-icon.svg" alt="water" width={16} height={16} className="w-5 h-5 text-secondary-foreground" />
            ...
            <CardFooter className="p-0 bordr-0 border-t-0 bg-transparent pt-8 flex flex-row gap-2">
                {product.volumes?.length > 0 && (
                    <Select value={product.selected_volume_id} disabled={!product.in_stock}>
                ...
                <Button variant="accent" disabled={!product.in_stock} size="lg" className="flex-1 gap-3 flex items-center justify-center">
